...
class GRTDriverStation(QMainWindow):
    ...
    def __init__(self):
        ...
        # Set the UI's central widget to a widget, and set that widget's
        # layout to the whole UI
        self.centralWidget = QWidget(self)
        self.setCentralWidget(self.centralWidget)
        self.centralWidget.setLayout(self.mainLayout)

        # The control panel is used for holistic settings across the UI (e.g.
        # alliance color, auton selection)
        ...
        # Since it's a horizontal layout, we're adding elements in order of
        # left to right
        # The control panel goes on the left, followed by the content layout
        # with everything else
        self.mainLayout.addWidget(self.controlPanel)
        self.mainLayout.addLayout(self.contentLayout)

        ''' 
        Everything that's not the control panel will end up in one of four (
        ?) tabs:
            1. Auton - Visualizing the autonomous paths, possibly plotting 
            and/or tuning?
            2. Match - Telemetry and visualizations, driver inputs (MOST 
            IMPORTANT)
            3. Debug - Testing/Fixing stuff in the pits (e.g. testing swerve, 
            simulating auton)
            4. Vision - Testing/Fixing vision (apriltags + driver cams) in 
            the pits (e.g. 'are tags detected?')
        ...
        actions_info = [
            ...
            "table_name": "PhotonVision", "entry_name": "xPosFront", "row": 
            0, "col": 0},
        ...
        """
        actions_info = [
            {"action_name": "GO TO AMP", "alliance": self.alliance, "row": 0, 
            "col": 0, "table_name": "elevator", "entry_name": 
            "target_location"},
            {"action_name": "GO TO SPEAKER", "alliance": self.alliance, 
            "row": 0, "col": 1, "table_name": "elevator", "entry_name": 
            "target_location"},
            {"action_name": "GO TO SOURCE", "alliance": self.alliance, 
            "row": 0, "col": 2, "table_name": "elevator", "entry_name": 
            "target_location"},
            {"action_name": "SCORE AMP", "alliance": self.alliance, "row": 0, 
            "col": 2, "table_name": "elevator", "entry_name": 
            "target_location"},
            {"action_name": "SCORE SPEAKER", "alliance": self.alliance, 
            "row": 0, "col": 2, "table_name": "elevator", "entry_name": 
            "target_location"},
        ...
        actions_info = [
            ...
            {"action_name": "GO TO SOURCE", "alliance": self.alliance, "row": 0,
             "col": 2,
             "table_name": "", "entry_name": "", "message": ""},
            {"action_name": "Go TO AMP", "alliance": self.alliance, "row": 1,
             "col": 0,
        ...
        self.controlPanel.alliance_color_changed.connect(
            self.actionGrid.change_alliance_color
        )
        self.controlPanel.alliance_color_changed.connect(
            self.map_widget.change_alliance_color
        )
        ...
        self.telem_widget_data_preset = [
            {"name": "Front x-pos", "value": "0.00",
             "table_name": "PhotonVision", "entry_name": "xPosFront", "row": 0,
             "col": 0},
            {"name": "Front y-pos", "value": "0.00",
             "table_name": "PhotonVision", "entry_name": "yPosFront", "row": 0,
             "col": 1},
            {"name": "Front Heading", "value": "0.00",
             "table_name": "PhotonVision", "entry_name": "headingFront",
             "row": 0, "col": 2},
            {"name": "Back x-pos", "value": "0.00",
             "table_name": "PhotonVision", "entry_name": "xPosBack", "row": 1,
             "col": 0},
            {"name": "Back y-pos", "value": "0.00",
             "table_name": "PhotonVision", "entry_name": "yPosBack", "row": 1,
             "col": 1},
            {"name": "Back Heading", "value": "0.00",
             "table_name": "PhotonVision", "entry_name": "headingBack",
             "row": 1, "col": 2}
            # {"name": "Quantity", "value": "0.00", "table_name":
            # "PhotonVision", "entry_name": "headingBack","row": 2, "col": 0},
            # {"name": "Quantity", "value": "0.00", "table_name":
            # "PhotonVision", "entry_name": "headingBack", "row": 2, "col": 1},
            # {"name": "Quantity", "value": "0.00", "table_name":
            # "PhotonVision", "entry_name": "headingBack", "row": 2, "col": 2}
        ]

        self.telem_grid = TelemGrid(self.telem_widget_data_preset)

        self.toggle_widget_data_preset = [
            {"name": "Vision", "value": True, "states": ('Enabled', 'Disabled'),
             "colors": ('green', 'red'), "row": 0, "col": 0},
            {"name": "Relative to", "value": True,
             "states": ('Field ', 'Robot'), "colors": ('green', 'red'),
             "row": 0, "col": 1},
            {"name": "Debug3", "value": True, "states": (True, False),
             "table_name": "PoseEstimates", "entry_name": "debugEnabled",
             "row": 0, "col": 2},
        ...
        # Add mapWidget, cameraAndActionLayout, and telem_toggle_layout to
        # match tab.
        ...
        self.crosshair1.setAttribute(
            QtCore.Qt.WidgetAttribute.WA_TransparentForMouseEvents
        )
        self.crosshair1.setPixmap(
            QPixmap(f"{os.path.dirname(__file__)}/crosshair.png")
        )
        self.crosshair1.hide()

        self.robot = QLabel(self.matchTab)
        self.robot.setObjectName("robot")
        self.robotPixmap = QPixmap(
            f"{os.path.dirname(__file__)}/RobotArrow.png"
        )
        ...
        rotation_angle = 90 if self.alliance == "red" else -90  # Choose the
        # rotation angle based on the alliance

        # self.field = mapLabel(self.match_tab, self)
        # self.field.setObjectName("field")
        # self.fieldPixmap = QPixmap(f"{os.path.dirname(
        # __file__)}/field24.png").scaled(600, 300)
        # self.field.setPixmap(self.fieldPixmap)
        # self.fieldOffsetX = 20
        # self.fieldOffsetY = 74
        # self.field.setGeometry(QtCore.QRect(self.fieldOffsetX,
        # self.fieldOffsetY, 600, 300))
        ...
        self.visionSwitch.setGeometry(QRect(1850, 80, self.QCheckBoxWidth, 
        self.QCheckBoxHeight))
        '''
    ...
    def getClickPosition(self, pos):
        ...
        self.crosshair1.setGeometry(
            pos.x() - 9 + self.fieldOffsetX, pos.y() - 9 + self.fieldOffsetY,
            20, 20
        )
